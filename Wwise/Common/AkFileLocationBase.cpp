	...
	)
{
	...
	if ( ( uiPathSize + MAX_FILETITLE_SIZE ) <= AK_MAX_PATH )
	{
		...
		if ( in_pFlags->uCodecID == AKCODECID_BANK )
			OS_PRINTF( pszTitle, ID_TO_STRING_FORMAT_BANK, (unsigned int)in_fileID );
		else if ( in_pFlags->uCodecID == AKCODECID_XMA )
			OS_PRINTF( pszTitle, ID_TO_STRING_FORMAT_XMA, (unsigned int)in_fileID );
		else if ( in_pFlags->uCodecID == AKCODECID_VORBIS )
			OS_PRINTF( pszTitle, ID_TO_STRING_FORMAT_VORBIS, (unsigned int)in_fileID );
		else
			OS_PRINTF( pszTitle, ID_TO_STRING_FORMAT_WAV, (unsigned int)in_fileID );
	}
	else
	{
