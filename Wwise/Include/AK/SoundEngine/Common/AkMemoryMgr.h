...
{   
...
#if defined(AK_WIN) || defined(XBOX360)
	/// External allocation hook for the Memory Manager. Called by the Audiokinetic 
	...
	namespace MemoryMgr
	{
		...
		/// \sa
		/// - \ref memorymanager
		extern AKMEMORYMGR_API wchar_t* GetPoolName( 
			AkMemPoolId     in_poolId			///< ID of memory pool
			);
		...
		/// - \ref memorymanager
	    extern AKMEMORYMGR_API void GetPoolMemoryUsed(
			AkMemPoolId     in_poolId,			///< ID of memory pool
		...
		/// \sa
		/// - \ref memorymanager
	    extern AKMEMORYMGR_API void * dMalign(
			AkMemPoolId in_poolId,				///< ID of the memory pool
		    size_t		in_uSize,				///< Number of bytes to allocate
