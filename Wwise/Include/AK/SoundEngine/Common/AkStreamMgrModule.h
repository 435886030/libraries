...
struct AkDeviceSettings
{
	...
	AkUInt32			uIOMemorySize;				///< Size of memory pool for I/O (for automatic streams). It is passed directly to AK::MemoryMgr::CreatePool().
	AkUInt32			uIOMemoryAlignment;			///< I/O memory pool alignment. It is passed directly to AK::MemoryMgr::CreatePool().
	AkMemPoolAttributes ePoolAttributes;			///< Attributes for internal IO memory pool. Note that these pool are always allocated internally as AkFixedSizeBlocksMode-style pools. Here, specify the block allocation type (AkMalloc, and so on). It is passed directly to AK::MemoryMgr::CreatePool().
...
/// Requests to Low-Level IO are synchronous. Calls the synchronous overloads of AK::IAkLowLevelIO::Read() and AK::IAkLowLevelIO::Write().
/// These methods should return only when the transfer is complete.
#define AK_SCHEDULER_BLOCKING          (0x01)
/// Requests to Low-Level IO are asynchronous, but posted one after the other. The I/O device thread selects the task 
/// which needs data the most, and calls the asynchronous overloads of AK::IAkLowLevelIO::Read() and AK::IAkLowLevelIO::Write().
...
/// Structure for synchronous transfers handshaking with the Low-Level IO. Used with blocking I/O hooks.
...
struct AkIOTransferInfo
{
    ...
    AkUInt32		uSizeTransferred;	///< Number of bytes that were transferred.
};
...
/// Structure for asynchronous transfers handshaking with the Low-Level IO. Extends AkIOTransferInfo.
/// \sa 
/// - AK::StreamMgr::IAkIOHookDeferred
/// - AkIOTransferInfo
/// - AkAIOCallback
...
namespace AK
{
	...
	namespace StreamMgr
	{
		...
		public:
			...
			/// (if you can guarantee that the I/O buffer will not be accessed in the meantime), but you may not wait here 
		...
		extern AKSTREAMMGR_API IAkStreamMgr * Create( 
		...
		/// - AkStreamMgrSettings
		/// - \ref streamingmanager_highlevel_defmanagerinit
		extern AKSTREAMMGR_API void GetDefaultSettings(
		...
		/// - AK::StreamMgr::SetFileLocationResolver()
		extern AKSTREAMMGR_API IAkFileLocationResolver * GetFileLocationResolver();

		/// Register the one and only File Location Resolver to the Stream Manager.
		/// \sa 
		/// - AK::StreamMgr::IAkFileLocationResolver
		extern AKSTREAMMGR_API void SetFileLocationResolver(
		...
		/// - \ref streamingmanager_highlevel_defmanagerinit
		extern AKSTREAMMGR_API AkDeviceID CreateDevice(
		...
		extern AKSTREAMMGR_API AKRESULT   DestroyDevice(
		...
		/// - AkDeviceSettings
		/// - \ref streamingmanager_highlevel_defmanagerinit
		extern AKSTREAMMGR_API void GetDefaultDeviceSettings(
